...
if doFitTP
    ...
    time_fit_begin=data(index_time_fit_begin, nbSweeps+2);
    
    
    %time when to stop the fit
    
    time_fit_end = data(1, nbSweeps+2) + time_points(2,1) + 0.5*(time_points(3,1)-time_points(2,1)); % offset "data(1, nbSweeps+2)" in case the data doesn't start at t=0
    ...
    time_min = data(index_min, nbSweeps+2);
...
if doPlot
    ...
    hold on, plot(data(:, nbSweeps+2), data(:, nbSweeps+1),'r');
    hold on, legend( 'Mean sweep' );
    
    % points reperes
    
    plot(data(time_points(1,2),nbSweeps+2), data(time_points(1,2),nbSweeps+1), 'bo') ;
    plot(data(time_points(2,2),nbSweeps+2), data(time_points(2,2),nbSweeps+1), 'bo') ;
    plot(data(time_points(3,2),nbSweeps+2), data(time_points(3,2),nbSweeps+1), 'bo') ;
